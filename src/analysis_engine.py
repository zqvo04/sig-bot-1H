"""
analysis_engine.py — 분석 엔진 1h (v3.5)
────────────────────────────────────────────────────────────────────
[v3.5 변경]
...
# ── analysis_engine.py 추가 함수 및 run_full_analysis 수정분 ──────
# 기존 파일에서 추가/수정이 필요한 부분만 발췌
# 위치: analyze_vol_price_divergence() 바로 아래, run_full_analysis() 위에 삽입
...
def analyze_daily_bias(df_1d):
    ...
    threshold_adj:
      방향 일치 → DAILY_BIAS_THRESHOLD_ADJ_ALIGN   (-3pt)
      역추세    → DAILY_BIAS_THRESHOLD_ADJ_AGAINST (+7pt)
    """
    _neutral = {
        "bias": "NEUTRAL",
    ...
    try:
        ...
        if bull_count >= 2:
            bias  = "BULL"
            adj_l = config.DAILY_BIAS_THRESHOLD_ADJ_ALIGN
            adj_s = config.DAILY_BIAS_THRESHOLD_ADJ_AGAINST
        elif bear_count >= 2:
            bias  = "BEAR"
            adj_l = config.DAILY_BIAS_THRESHOLD_ADJ_AGAINST
            adj_s = config.DAILY_BIAS_THRESHOLD_ADJ_ALIGN
        else:
            bias  = "NEUTRAL"
            adj_l = adj_s = 0
