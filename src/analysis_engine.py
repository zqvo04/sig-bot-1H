...
def classify_market_regime(df_1h, adx, bb):
    ...
    ma20_cross=0; er=1.0; er_pctl=None; slope_persist=0.0
    try:
        ...
        # [Pillar1-②] 방향지속 — MA20 기울기 부호 일관성(느린 grind 핵심 판별자, 방향무관)
    ...
    # 승격 신호(둘 다 방향무관 → 상승/하락 grind 대칭)
    slope_sig=(getattr(config,"WRF_REGIME_SLOPE_PERSIST",False)
               and slope_persist>=getattr(config,"WRF_REGIME_SLOPE_MIN_FRAC",0.70))
    if getattr(config,"WRF_REGIME_ER_TREND",False):
        er_sig=((er_pctl is not None and er_pctl>=getattr(config,"WRF_REGIME_ER_PCTL_MIN",0.70))
                if getattr(config,"WRF_REGIME_ER_PCTL",False)
                else er>=getattr(config,"WRF_REGIME_ER_TREND_MIN",0.50))
    else: er_sig=False
